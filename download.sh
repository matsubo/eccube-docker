#!/bin/bash

#wget http://downloads.ec-cube.net/src/eccube-2.11.0.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.11.1.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.11.2.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.11.3.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.11.4.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.11.5.tar.gz

# 2.12.3 EC-CUBE plugin used helper function added from 2.12.3
#wget http://downloads.ec-cube.net/src/eccube-2.12.0.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.12.1.tar.gz
#wget http://downloads.ec-cube.net/src/eccube-2.12.2.tar.gz


wget http://downloads.ec-cube.net/src/eccube-2.12.3.tar.gz
wget http://downloads.ec-cube.net/src/eccube-2.12.4.tar.gz
wget http://downloads.ec-cube.net/src/eccube-2.12.5.tar.gz
wget http://downloads.ec-cube.net/src/eccube-2.12.6.tar.gz

wget http://downloads.ec-cube.net/src/eccube-2.13.0.tar.gz
wget http://downloads.ec-cube.net/src/eccube-2.13.1.tar.gz
wget http://downloads.ec-cube.net/src/eccube-2.13.2.tar.gz
wget http://downloads.ec-cube.net/src/eccube-2.13.3.tar.gz


